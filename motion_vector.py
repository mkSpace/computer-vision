...
# 이미지를 불러온다.
color_first_img = cv2.imread("img/img0.png", 1)
color_second_img = cv2.imread("img/img1.png", 1)
first_img = cv2.imread("img/img0.png", 0)
second_img = cv2.imread("img/img1.png", 0)
...
for y in range(n, height - n):
    for x in range(n, width - n):
        try:
            ...
            # A, v, b 행렬을 구성한다.
            A = np.array(cv2.hconcat([np.reshape(y_tmp, (N_SIZE ** 2, 1)), np.reshape(x_tmp, (N_SIZE ** 2, 1))]))
            A_T = A.T
